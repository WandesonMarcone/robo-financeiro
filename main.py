...
def atualizar_financeiro(request):
    ...
    # 1. RASPAGEM E SELEÇÃO (A lógica que estava no código longo)
    try:
        ...
        if comando == "PESQUISAR":
            # Filtro: P/L > 0, ROE > 10% e NÃO está no core
            ...
            # Rotatividade: pega os 5 mais antigos da planilha (Lógica preservada)
    ...
    # 2. MOTOR DE ATUALIZAÇÃO E AUDITORIA (Mantendo a estrutura robusta)
    lote_updates = []
    dados_json_global = {}
    ...
    for ticker in ativos_finais:
        try:
            ...
            # JSON AG1 (Preservado)
            dados_json_global[ticker] = {"status": status_msg}
