...
def atualizar_financeiro():
    ...
    # IGNORA ERROS NA PLANILHA: Pula números perdidos na Coluna A
    ...
    cat_metodologia = [ticker_c3] if ticker_c3 and ticker_c3 in todas and ticker_c3 not in cat_fixas else []

    # --- 3. Oportunidades ---
    opps_brutas = df_filtros[(df_filtros['P/L'] > 0.1) & (df_filtros['P/L'] < 12) & (df_filtros['P/VP'] < 1.5)].index.tolist()
    cat_opps = [o for o in opps_brutas if o in todas and o not in cat_fixas and o not in cat_metodologia][:5]
    ...
    # Log Limpo e Detalhado
    print(f"-> Ações Fixas: {cat_fixas}")
    if cat_metodologia: print(f"-> Metodologia (C3): {cat_metodologia}")
    ...
    for ticker in fila:
        ...
        try:
            ...
            # --- PADRÃO ORIGINAL RESTAURADO (EXPLICITO B até AF) ---
            row_base = [
                preco,                                    # B: Preço (YF)
            ...
            # --- TRAVA DE SEGURANÇA DE COLUNA ---
            if ticker in cat_novatas:
                # É nova ação: Escreve da Coluna A até AF (Adiciona o Ticker no começo da lista)
                row_final = [ticker] + row_base
                range_update = f'A{linha_idx}:AF{linha_idx}'
            else:
                # Já existe na planilha: Escreve da Coluna B até AF (Protege a Coluna A)
                row_final = row_base
                range_update = f'B{linha_idx}:AF{linha_idx}'

            batch_updates.append({'range': range_update, 'values': [row_final]})

            # Log Individual Simplificado
    ...
    if batch_updates:
        ...
        if cat_aleatorias: msg_wpp += f"🎲 *Atualização Aleatórias:*\n{', '.join(cat_aleatorias)}\n\n"
        if relatorio_opps: msg_wpp += "🎯 *Oportunidades Atualizadas:*\n" + "\n".join(relatorio_opps) + "\n\n"
        if relatorio_novatas: msg_wpp += "🌟 *NOVA AÇÃO ADICIONADA:*\n" + "\n".join(relatorio_novatas)
...
if __name__ == "__main__":
    atualizar_financeiro()rllib.parse
from datetime import datetime
