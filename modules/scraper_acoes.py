...
def rodar_garimpo_acoes(planilha, agora_dt, agora_sp, sp_tz):
    ...
    print("[3/5] Organizando a Fila de Ações (Trava de Tempo)...")
    ...
    print(f"-> Ações Fixas na Fila: {cat_fixas} | Total Fila: {len(fila)}")

    print("[4/5] Processando cruzamento Yahoo Finance linha a linha...")
    batch_updates = []
    relatorio_opps = []
    relatorio_fixas = [] 
    relatorio_novatas = []
        ...
        try:
            ...
            if ticker in opps_brutas:
                ...
                if ticker in config.FIXAS_ACOES:
                    relatorio_fixas.append(f"🟢 *Sinal Verde em {ticker}*\n   Preço: R$ {preco} | P/VP: {pvp_wpp}\n   ROE: {roe_wpp:.1f}%")
                else:
                    relatorio_opps.append(f"• *{ticker}*: {detalhe_msg}")

            if ticker in cat_novatas:
                dy_wpp = formatar(f.get('Div.Yield', 0)) * 100
                relatorio_novatas.append(f"• *{ticker}*: R$ {preco} | 🏭 {setor} (DY: {dy_wpp:.1f}%)")

        except Exception as e:
            print(f"   ❌ [ERRO] Falha ao processar {ticker}: {e}")

    # Montagem do relatório textual para envio posterior
    msg_out = ""
    if relatorio_fixas or relatorio_opps or relatorio_novatas:
        msg_out += "🤖 *Caçador de Ações* 🤖\n\n"
        if relatorio_fixas:
            msg_out += "🚨 *ALERTA VIP: ACOES FIXAS BARATAS* 🚨\n" + "\n".join(relatorio_fixas) + "\n\n"
        if relatorio_opps: 
            msg_out += "🎯 *Oportunidades (P/VP < 1.5):*\n" + "\n".join(relatorio_opps) + "\n\n"
        if relatorio_novatas: 
            msg_out += "🌟 *NOVAS EMPRESAS CAÇADAS:*\n" + "\n".join(relatorio_novatas)
