...
def rodar_garimpo_fiis(planilha, agora_dt, agora_sp, sp_tz):
    ...
    try:
        ...
        print(f"❌ Erro Fundamentus FIIs (Timeout ou Queda): {e}")
        df = pd.DataFrame() 
    ...
    if not fila_total: 
        print("✅ [FIIs] Nenhuma atualização necessária (Trava de 2h ativa).")
        return [], "", aba_fiis
    ...
    for ticker in fila_total:
        try:
            ...
            # =========================================================================
            # 🗺️ MAPEAMENTO COMPLETO DAS 16 COLUNAS DA ABA "BD_FIIs" (COLUNAS A ATÉ Q)
            # =========================================================================
            row_update_completo = [
                ticker,                 # 🟢 Coluna A: Ticker do Ativo
                tipo,                   # 🟢 Coluna B: Tipo de FII 
                setor,                  # 🟢 Coluna C: Segmento do Fundo 
                preco,                  # 🔵 Coluna D: Cotação Atual 
                numero_cotas,           # 🧮 Coluna E: Quantidade de Cotas 
                pvp,                    # 🟢 Coluna F: P/VP Real 
                dy,                     # 🟢 Coluna G: Dividend Yield 12 Meses 
                vacancia,               # 🟢 Coluna H: Vacância Média 
                qtd_imoveis,            # 🟢 Coluna I: Quantidade Física de Imóveis 
                "Mapeamento em Curso",  # ⚪ Coluna J: WALT 
                "Pendente",             # ⚪ Coluna K: Alavancagem / Dívida 
                liquidez,               # 🟢 Coluna L: Liquidez Média Diária 
                valor_mercado,          # 🟢 Coluna M: Patrimônio Líquido 
                vpa,                    # 🧮 Coluna N: Valor Patrimonial da Cota 
                lucro_12m,              # 🧮 Coluna O: Lucro Total Distribuído 12M 
                media_div_mensal,       # 🧮 Coluna P: Projeção de Dividendo Mensal/Cota 
                f"{agora_sp} OK"        # ⏰ Coluna Q: Carimbo de Data/Hora 
            ]
            ...
            if ticker in config.FIXAS_FIIS:
                txt = f"{emoji} *{ticker}* ({tipo})\n   R$ {preco_velho:.2f} ➔ R$ {preco:.2f} {icone_variacao}\n   P/VP: {pvp:.2f} | DY: {dy*100:.1f}%{txt_vacancia}"
                if ticker in novatos_garimpados: # Você considera novatos_garimpados como regra de oportunidade geral
    ...
    if relatorio_atualizados: msg_blocos.append("🔄 *ATUALIZAÇÕES:*\n" + "\n\n".join(relatorio_atualizados))
